...
int main() {
    ...
    if (caseID == 1) {
        ...
        Entry entry;
        entry.setPrimitiveField<string>("name", "DatabaseEntry");
        ...
        HashTable<Entry*>* hashTableinIndex = index_tree.get("DatabaseEntry");
        cout << "DatabaseEntry gets added to index tree: " << boolalpha << (hashTableinIndex != nullptr) << endl;
        if (hashTableinIndex != nullptr) {
            cout << "Size of the HashTable under the key DatabaseEntry: " << hashTableinIndex->size() << endl;
        }
